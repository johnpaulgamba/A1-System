...
Public Class PaymentMethodRegistration
    ...
    Private Sub tostartNewRecord()
        toSave = True
        With Me
            .Text = "PaymentMethod Entry"
            .Status.Checked = True
            .PaymentMethod.Text = Nothing
    ...
    Public Sub EditLoad(ByVal a As Integer)
        loadDS("SELECT * from payroll_PaymentMethod where PayMethodID='" & a & "'")
    End Sub
    ...
    Private Sub SaveRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        Try
            If Trim(Me.PaymentMethod.Text) = "" Then MessageBox.Show("PaymentMethod is required!", "Required field", MessageBoxButtons.OK, MessageBoxIcon.Stop) : Me.PaymentMethod.Focus() : Exit Sub
            ...
            If toSave = True Then
                If RecordDoExists("SELECT * from payroll_PaymentMethod where PaymentMethod='" & Me.PaymentMethod.Text & "'") = True Then
                    MessageBox.Show("The PaymentMethod exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            Else
                If RecordDoExists("SELECT * from payroll_PaymentMethod where PaymentMethod='" & Me.PaymentMethod.Text & "' AND PayMethodID<> '" & Me.PayMethodID & "'") = True Then
                    MessageBox.Show("The PaymentMethod exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            End If
            If toSave = True Then
                saveEditDelete("INSERT into payroll_PaymentMethod (PaymentMethod, Description,  Active, Creator, ChangesMade) VALUES ( '" & _
                                    convertQuotes(Me.PaymentMethod.Text) & _
                                 "', '" & convertQuotes(Me.Description.Text) & _
                                  "', '" & Convert.ToSingle(Me.Status.Checked) & _
                                   "', '" & APCESMainForm.UserFullName.Text & _
                                  "', 'Originally created: " & convertQuotes(PaymentMethod.Text) & vbCrLf & _
                                 ...
                                 convertQuotes(APCESMainForm.ComputerName.Text) & _
                                  "')", "saved")
            Else
                saveEditDelete("update payroll_PaymentMethod SET PaymentMethod= '" & _
                                    convertQuotes(Me.PaymentMethod.Text) & _
                                 "', Description='" & convertQuotes(Me.Description.Text) & _
                                 "', Active='" & Convert.ToSingle(Me.Status.Checked) & _
                                 "', ChangesMade=Concat('Updated to: " & MySqlHelper.EscapeString(Me.PaymentMethod.Text.ToUpper) & vbCrLf & _
                                  "Active:" & Me.Status.Checked & vbCrLf & _
                                  "by " & _
                                  APCESMainForm.UserFullName.Text & " at " & Now & _
                                  "@ " & APCESMainForm.ComputerName.Text & _
                                  vbCrLf & vbCrLf & "-->'" & _
                                  ", ChangesMade) WHERE PayMethodID='" & Me.PayMethodID & "'", "updated..")
            End If
            ...
            If IsNotError = False Then
                ...
                PaymentMethodMainForm.loadingPaymentMethods(QuerytoString)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Function RecordDoExists(ByVal a As String) As Boolean
        ...
        End Try
    End Function
    Private Sub PaymentMethod_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles PaymentMethod.Validating
        If toSave = True Then
            If RecordDoExists("SELECT * from payroll_PaymentMethod where PaymentMethod='" & Me.PaymentMethod.Text & "'") = True Then
                MessageBox.Show("The PaymentMethod exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                e.Cancel = True
