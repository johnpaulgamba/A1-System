...
Public Class PayrollGroupRegistration
    ...
    Private Sub tostartNewRecord()
        toSave = True
        With Me
            .Text = "Payroll Group Entry"
            .Status.Checked = True
            .PayrollGroup.Text = Nothing
    ...
    Public Sub EditLoad(ByVal a As Integer)
        loadDS("SELECT * from Payroll_Group where PayGroupID='" & a & "'")
    End Sub
    ...
    Private Sub SaveRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        Try
            If Trim(Me.PayrollGroup.Text) = "" Then MessageBox.Show("PayrollGroup is required!", "Required field", MessageBoxButtons.OK, MessageBoxIcon.Stop) : Me.PayrollGroup.Focus() : Exit Sub
            ...
            If toSave = True Then
                If RecordDoExists("SELECT * from Payroll_Group where PayrollGroup='" & Me.PayrollGroup.Text & "'") = True Then
                    MessageBox.Show("The PayrollGroup exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            Else
                If RecordDoExists("SELECT * from Payroll_Group where PayrollGroup='" & Me.PayrollGroup.Text & "' AND PayGroupID<> '" & Me.PayGroupID & "'") = True Then
                    MessageBox.Show("The PayrollGroup exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            End If
            If toSave = True Then
                saveEditDelete("INSERT into Payroll_Group (PayrollGroup, Description,  Active, Creator, ChangesMade) VALUES ( '" & _
                                    convertQuotes(Me.PayrollGroup.Text) & _
                                 "', '" & convertQuotes(Me.Description.Text) & _
                                  "', '" & Convert.ToSingle(Me.Status.Checked) & _
                                   "', '" & APCESMainForm.UserFullName.Text & _
                                  "', 'Originally created: " & convertQuotes(PayrollGroup.Text) & vbCrLf & _
                                 ...
                                 convertQuotes(APCESMainForm.ComputerName.Text) & _
                                  "')", "saved")
            Else
                saveEditDelete("update Payroll_Group SET PayrollGroup= '" & _
                                    convertQuotes(Me.PayrollGroup.Text) & _
                                 "', Description='" & convertQuotes(Me.Description.Text) & _
                                 "', Active='" & Convert.ToSingle(Me.Status.Checked) & _
                                 "', ChangesMade=Concat('Updated to: " & MySqlHelper.EscapeString(Me.PayrollGroup.Text.ToUpper) & vbCrLf & _
                                  "Active:" & Me.Status.Checked & vbCrLf & _
                                  "by " & _
                                  APCESMainForm.UserFullName.Text & " at " & Now & _
                                  "@ " & APCESMainForm.ComputerName.Text & _
                                  vbCrLf & vbCrLf & "-->'" & _
                                  ", ChangesMade) WHERE PayGroupID='" & Me.PayGroupID & "'", "updated..")
            End If
            ...
            If IsNotError = False Then
                ...
                PayrollGroupMainForm.loadingPayrollGroups(QuerytoString)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Function RecordDoExists(ByVal a As String) As Boolean
        ...
        End Try
    End Function
    Private Sub PayrollGroup_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles PayrollGroup.Validating
        If toSave = True Then
            If RecordDoExists("SELECT * from Payroll_Group where PayrollGroup='" & Me.PayrollGroup.Text & "'") = True Then
                MessageBox.Show("The PayrollGroup exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                e.Cancel = True
