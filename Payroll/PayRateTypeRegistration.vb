...
Public Class PayRateTypeRegistration
        ...
        Try
            ...
            If dsItem.Tables(0).Rows.Count <= 1 Then
                With dsItem.Tables(0).Rows(0)
                    ...
                    Me.PayRateType.Text = .Item("PayRateType")
    ...
    Private Sub tostartNewRecord()
        toSave = True
        With Me
            .Text = "PayRateType Entry"
            .Status.Checked = True
            .PayRateType.Text = Nothing
    ...
    Public Sub EditLoad(ByVal a As Integer)
        loadDS("SELECT * from Payroll_PayRateType where PayRateID='" & a & "'")
    End Sub
    ...
    Private Sub SaveRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        Try
            If Trim(Me.PayRateType.Text) = "" Then MessageBox.Show("PayRateType is required!", "Required field", MessageBoxButtons.OK, MessageBoxIcon.Stop) : Me.PayRateType.Focus() : Exit Sub
            ...
            If toSave = True Then
                If RecordDoExists("SELECT * from Payroll_PayRateType where PayRateType='" & Me.PayRateType.Text & "'") = True Then
                    MessageBox.Show("The PayRateType exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            Else
                If RecordDoExists("SELECT * from Payroll_PayRateType where PayRateType='" & Me.PayRateType.Text & "' AND PayRateID<> '" & Me.PayRateID & "'") = True Then
                    MessageBox.Show("The PayRateType exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            End If
            If toSave = True Then
                saveEditDelete("INSERT into Payroll_PayRateType (PayRateType, Description,  Active, Creator, ChangesMade) VALUES ( '" & _
                                    convertQuotes(Me.PayRateType.Text) & _
                                 "', '" & convertQuotes(Me.Description.Text) & _
                                  "', '" & Convert.ToSingle(Me.Status.Checked) & _
                                   "', '" & APCESMainForm.UserFullName.Text & _
                                  "', 'Originally created: " & convertQuotes(PayRateType.Text) & vbCrLf & _
                                 ...
                                 convertQuotes(APCESMainForm.ComputerName.Text) & _
                                  "')", "saved")
            Else
                saveEditDelete("update Payroll_PayRateType SET PayRateType= '" & _
                                    convertQuotes(Me.PayRateType.Text) & _
                                 "', Description='" & convertQuotes(Me.Description.Text) & _
                                 "', Active='" & Convert.ToSingle(Me.Status.Checked) & _
                                 "', ChangesMade=Concat('Updated to: " & MySqlHelper.EscapeString(Me.PayRateType.Text.ToUpper) & vbCrLf & _
                                  "Active:" & Me.Status.Checked & vbCrLf & _
                                  "by " & _
                                  APCESMainForm.UserFullName.Text & " at " & Now & _
                                  "@ " & APCESMainForm.ComputerName.Text & _
                                  vbCrLf & vbCrLf & "-->'" & _
                                  ", ChangesMade) WHERE PayRateID='" & Me.PayRateID & "'", "updated..")
            End If
            ...
            If IsNotError = False Then
                ...
                PayRateTypeMainForm.loadingPayRateTypes(QuerytoString)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Function RecordDoExists(ByVal a As String) As Boolean
        ...
        End Try
    End Function
    Private Sub PayRateType_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles PayRateType.Validating
        If toSave = True Then
            If RecordDoExists("SELECT * from Payroll_PayRateType where PayRateType='" & Me.PayRateType.Text & "'") = True Then
                MessageBox.Show("The PayRateType exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                e.Cancel = True
