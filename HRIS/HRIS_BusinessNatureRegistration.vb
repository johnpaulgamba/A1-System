...
Public Class HRIS_BusinessNatureRegistration
    ...
    Public BusinessNatureID As Integer
        ...
        Try
            ...
            If dsItem.Tables(0).Rows.Count <= 1 Then
                With dsItem.Tables(0).Rows(0)
                    ...
                    Me.BusinessNature.Text = .Item("BusinessNature")
    ...
    Private Sub tostartNewRecord()
        toSave = True
        With Me
            .Text = "BusinessNature Entry"
            .Status.Checked = True
            .BusinessNature.Text = Nothing
    ...
    Public Sub EditLoad(ByVal a As Integer)
        loadDS("SELECT * from HRIS_BusinessNature where BusinessNatureID='" & a & "'")
    End Sub
    ...
    Private Sub SaveRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        Try
            If Trim(Me.BusinessNature.Text) = "" Then
                MessageBox.Show("BusinessNature is required!", "Required field", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                Exit Sub
            End If
            ...
            End If
            If toSave = True Then
                If RecordDoExists("SELECT * from HRIS_BusinessNature where BusinessNature='" & Me.BusinessNature.Text & "'") = True Then
                    MessageBox.Show("The BusinessNature exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            Else
                If RecordDoExists("SELECT * from HRIS_BusinessNature where BusinessNature='" & Me.BusinessNature.Text & "' AND BusinessNatureID<> '" & Me.BusinessNatureID & "'") = True Then
                    MessageBox.Show("The BusinessNature exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            End If
            If toSave = True Then
                saveEditDelete("INSERT INTO HRIS_BusinessNature (BusinessNature, Description,  Active, EmployeeID, DateCreated, CreatorNtUser, LastUpdate, ChangesMade) VALUES ( '" & _
                                    convertQuotes(Me.BusinessNature.Text) & _
                                 "', '" & convertQuotes(Me.Description.Text) & _
                                  ...
                                  "', 'NONE', 'Originally created: " & convertQuotes(BusinessNature.Text) & vbCrLf & _
                                 ...
                                 convertQuotes(APCESMainform.ComputerName.Text) & _
                                  "')", "saved")
            Else
                saveEditDelete("UPDATE HRIS_BusinessNature SET BusinessNature= '" & _
                                    convertQuotes(Me.BusinessNature.Text) & _
                                 "', Active='" & Convert.ToSingle(Me.Status.Checked) & _
                                  "', LastUpdate='" & Now & _
                                  "', ChangesMade=Concat('Updated to: BusinessNature:" & MySqlHelper.EscapeString(Me.BusinessNature.Text.ToUpper) & vbCrLf & _
                                  "Active:" & Me.Status.Checked & vbCrLf & _
                                  "by " & _
                                  APCESMainform.EmployeeName.Text & " at " & Now & _
                                  "@ " & APCESMainform.ComputerName.Text & _
                                  vbCrLf & vbCrLf & "-->'" & _
                                  ", ChangesMade) WHERE BusinessNatureID='" & Me.BusinessNatureID & "'", "updated..")
            End If
            ...
            If IsNotError = False Then
                ...
                HRIS_BusinessNatureMainForm.loadingHRIS_BusinessNature(QuerytoString)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Function RecordDoExists(ByVal a As String) As Boolean
        ...
        End Try
    End Function
    Private Sub Department_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles BusinessNature.Validating
        If toSave = True Then
            If RecordDoExists("SELECT * from HRIS_BusinessNature where BusinessNature='" & Me.BusinessNature.Text & "'") = True Then
                MessageBox.Show("The BusinessNature exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                e.Cancel = True
