...
Public Class Hris_Employee_TINMainForm
    Dim query1 As String = "Select a.EmployeeID,a.EmployeeNo, concat(a.lastname,', ',a.FirstName,' ',a.MiddleName) as Employee, a.TIN as TIN,format(a.HiddenAllowance,2) as On_Voucher, format(a.BasicSalary,2) as Basic_Salary, format(a.BasicSalary+a.HiddenAllowance,2) as Gross_Pay, " & _
    " format((a.sssee+a.phicee+a.hdmfee),2) as Mandatory, format(a.legalallowance,2) as Exemption," & _
    " format(@TaxableIncome:=a.BasicSalary-a.sssee-a.phicee-a.hdmfee-a.legalallowance,2) as Taxable_Income, " & _
    " format((case when @taxableincome > b.range_from then (((@taxableincome-b.range_from)*b.add_percent)+b.add_value) else b.add_value end),2) as WTAX,a.Creator" & _
    " from hris_employee a join taxtable b where b.PayrollGroup='MONTHLY' and a.basicsalary between b.range_from and b.range_to and a.active=1 "
    Dim query2 As String = " TIN"
...
#Region "Controls and Buttons"
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            Dim x As New HRIS_Employee_TIN_Registration(Me.DataGridView1.Item(i, "EmployeeID"))
            With x
                .StartPosition = FormStartPosition.CenterScreen
                .ShowDialog()
...
#End Region
    ...
    Public Sub formatdatagrid()
        ...
        With Me.DataGridView1
            ...
            .Cols("Gross_Pay").TextAlign = TextAlignEnum.RightCenter
            .Cols("Exemption").TextAlign = TextAlignEnum.RightCenter
            .Cols("Mandatory").TextAlign = TextAlignEnum.RightCenter
            .Cols("Taxable_Income").TextAlign = TextAlignEnum.RightCenter
            .Cols("Basic_Salary").TextAlign = TextAlignEnum.RightCenter
            .Cols("Witholding_TAX").TextAlign = TextAlignEnum.RightCenter
            .AllowSorting = True
            .AllowFiltering = True
            .AllowResizing = True
            .AutoSizeCols()
    ...
    Private Function returnActiveInactive(ByVal a As Boolean, ByVal b As Boolean, ByVal c As Boolean)
        If a = False And b = False Then Return "" Else Return " AND a.ACTIVE=" & Convert.ToSingle(a)
    End Function
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        If SSSNo.Text = "" Then s3 = "" Else s3 = " and a.TIN like '%" & SSSNo.Text & "%' "
    ...
    Private Sub ClearFields_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ClearFields.Click
        FunctionClass.ClearPanel(C1InputPanel1)
    End Sub
    ...
    Private Sub Hris_Employee_TINMainForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
