...
Imports System.IO
Public Class HRIS_TrainingsMainForm

    Dim query1 As String = " Select a.TrainingID as TrainingID, " & _
        " Concat('TRI ', a.TrainingID) as Reference, " & _
        " Concat('EMP ', b.EmployeeID) as EmployeeID,  " & _
        " Concat(LastName, ', ', FirstName, ' ', MiddleName) as Employee, " & _
        " CourseName, Date_from, Date_to, NoofHours as NoOfHours, " & _
        " a.ConductedBy, a.Creator " & _
        " From HRIS_Training a" & _
        " JOIN hris_employee b On a.EmployeeID=b.EmployeeID  " & _
        " WHERE a.EmployeeID=b.EmployeeID and b.companyid = '" & companyid & "' "
    Dim query2 As String = " CourseName  "
    Dim QUERY3 As String = " GROUP BY a.TrainingID "
...
#Region "Controls and Commands"
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            Dim ID As Integer = Me.DataGridView1.Rows(i).Item("TrainingID")
            Dim x As New HistoryUpdate
            With x
                .loaddata("HRIS_Training", "TrainingID", ID)
...
#End Region
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        If sssno.Text = "" Then s3 = "" Else s3 = " and a.coursename like '%" & sssno.Text & "%' "
    ...
    Private Sub EditCommand_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles EditCommand.Click
        Try
            ...
            Dim i As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("TrainingID")
            Dim x As New HRIS_EmployeeRegistration_Training(i)
            With x
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
