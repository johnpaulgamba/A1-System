...
Public Class HRIS_Employee_HDMF_Registration
    ...
    Public Sub loadpremium(ByVal salary As Double)
        Try
            ...
            command = New MySqlCommand
            With command
                .CommandText = "Select * from hdmftable where '" & Me.basicsalary.Value & "' between rangefrom and rangeto"
                .Connection = con
                reader = .ExecuteReader
            End With
            If reader.Read = True Then
                With reader
                    If .Item("Compute") = True Then
                        Me.eeshare.Value = reader.Item("hdmfEE") * .Item("SalaryCredit")
                        Me.ershare.Value = reader.Item("hdmfEr") * .Item("SalaryCredit")
                        Me.total.Value = eeshare.Value + ershare.Value
                    Else
                        Me.eeshare.Value = reader.Item("hdmfEE")
                        Me.ershare.Value = reader.Item("hdmfEr")
                        Me.total.Value = eeshare.Value + ershare.Value
                    End If
    ...
    Private Sub SaveCommandButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveCommandButton.Click
        saveEditDelete("update hris_employee set hdmf='" & Reference.Text & "', hdmfee='" & eeshare.Value & "',hdmfer='" & ershare.Value & "' where employeeid = '" & employeeid & "'", "saved")
    End Sub
End Class