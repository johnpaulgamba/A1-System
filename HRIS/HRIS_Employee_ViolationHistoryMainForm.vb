...
Imports System.IO
Public Class HRIS_ViolationHistoryMainForm

    Dim query1 As String = " Select a.TicketID, concat('',a.TicketNo) as Ticket_No,date_format(a.ViolationDate,'%m/%d/%Y') as ViolationDate, concat(d.LastName,', ',d.FirstName, ' ',d.MiddleName) as Employee, b.ViolationType,b.ViolationDetails,a.DateReceived," & _
                            " a.IssuedBy, a.Status, a.DateCreated,a.Creator from hris_violationticket a join hris_violation_setup b on a.violationid=b.violationid join hris_employee d on a.EmployeeID=d.employeeid where a.employeeid=d.employeeid  "
    Dim query2 As String = " a.ViolationDate  "
    ...
    Private Sub ClearFields_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ClearFields.Click
        FunctionClass.ClearPanel(c1inputpanel)
    End Sub
...
#Region "Controls and Commands"
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            Dim ID As Integer = Me.DataGridView1.Rows(i).Item("TicketID")
            Dim x As New HistoryUpdate
            With x
                .loaddata("HRIS_Memo", "TicketID", ID)
                .Text = "Item history of " & Me.DataGridView1.Item(i, "TicketID")
...
#End Region
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        Dim ViolationDateQuery As String = ""
        If Me.ViolationDateFrom.Text = Nothing And Me.ViolationDateTo.Text = Nothing Then
            ViolationDateQuery = ""
        Else
            If Me.ViolationDateFrom.Text <> Nothing Then
                If ViolationDateTo.Text <> Nothing Then
                    ViolationDateQuery = " AND Date_Format(b.ViolationDate, '%Y/%m/%d')  between '" & Format(ViolationDateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(ViolationDateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    ViolationDateQuery = " AND b.ViolationDate >= '" & Format(ViolationDateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If ViolationDateTo.Text <> Nothing Then
                    ViolationDateQuery = " AND b.ViolationDate <= '" & Format(ViolationDateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    ViolationDateQuery = " AND b.ViolationDate LIKE '%%' "
                End If
            End If
        End If
        ...
        Dim s3 As String = specificviolation.Text
        Dim s4 As String = EmployeeName.Text
        Dim s5 As String = generalviolation.Text
        Dim s6 As String = Status.Text

        If CompanyName.Text = "" Then s1 = "" Else s1 = " and d.companyid = '" & CompanyName.SelectedValue & "' "
        If Department.Text = "" Then s2 = "" Else s2 = " and d.departmentid = '" & Department.SelectedValue & "' "
        If EmployeeName.Text = "" Then s4 = "" Else s4 = " and a.employeeid='" & EmployeeName.SelectedValue & "' "
        If specificviolation.Text = "" Then s3 = "" Else s3 = " and b.violationtype like '%" & specificviolation.Text & "%' "
        If generalviolation.Text = "" Then s5 = "" Else s5 = " and b.violationdetails like '%" & generalviolation.Text & "%' "
        If Status.Text = "" Then s5 = "" Else s5 = " and a.status like '%" & Status.Text & "%' "
        Dim filtersearchquery As String = s1 & s2 & s3 & s4 & s4 & s5 & s6 & ViolationDateQuery
    ...
    Private Sub EditCommand_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles EditCommand.Click
        ...
        Dim i As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("TicketID")
        Dim x As New HRIS_EmployeeRegistration_ViolationRegistration(i)
        With x
            .StartPosition = FormStartPosition.CenterParent
            .ShowDialog()
        End With
