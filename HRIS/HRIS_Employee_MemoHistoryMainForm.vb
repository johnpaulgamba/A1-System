...
Imports System.IO
Public Class HRIS_MemoHistoryMainForm

    Dim query1 As String = " Select  concat('',a.MemoID) as Memo_No, concat('',a.TicketNo) as Ticket_No, date_format(a.MemoDate,'%m/%d/%Y') as MemoDate, concat(d.LastName,', ',d.FirstName, ' ',d.MiddleName) as Employee,b.ViolationType,b.ViolationDetails,c.PenaltyLevel, concat(c.NoOfDays,' DAYS') as NoOfDays,a.SuspensionDate,a.DateServed," & _
                            " a.IssuedBy, a.Status from hris_memo a join hris_violation_setup b on a.violationid=b.violationid join hris_penaltysetup c on a.penaltyid=c.penaltyid join hris_employee d on a.EmployeeID=d.employeeid where a.memoid<>0 "
    Dim query2 As String = " a.memodate  "
...
#Region "Controls and Commands"
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            Dim ID As Integer = Me.DataGridView1.Rows(i).Item("memoid")
            Dim x As New HistoryUpdate
            With x
                .loaddata("HRIS_Memo", "memoid", ID)
                .Text = "Item history of " & Me.DataGridView1.Item(i, "MemoID")
...
#End Region
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        Dim MemoDateQuery As String = ""
        If Me.MemoDateFrom.Text = Nothing And Me.MemoDateTO.Text = Nothing Then
            MemoDateQuery = ""
        Else
            If Me.MemoDateFrom.Text <> Nothing Then
                If MemoDateTO.Text <> Nothing Then
                    MemoDateQuery = " AND Date_Format(b.MemoDate, '%Y/%m/%d')  between '" & Format(MemoDateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(MemoDateTO.Value, "yyyy/MM/dd") & "' "
                Else
                    MemoDateQuery = " AND b.MemoDate >= '" & Format(MemoDateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If MemoDateTO.Text <> Nothing Then
                    MemoDateQuery = " AND b.MemoDate <= '" & Format(MemoDateTO.Value, "yyyy/MM/dd") & "' "
                Else
                    MemoDateQuery = " AND b.MemoDate LIKE '%%' "
                End If
            End If
        End If
        ...
        Dim filtersearchquery As String = s1 & s2 & s3 & s4 & s4 & s5 & s6 & MemoDateQuery
    ...
    Private Sub EditCommand_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles EditCommand.Click
        Try
            ...
            Dim i As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("memoid")
            Dim x As New HRIS_EmployeeRegistration_Memo(i)
            With x
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
