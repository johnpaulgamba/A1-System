...
Public Class DeliveryReceiptMain
    ...
    Dim query1 As String = "Select a.DeliveryReceiptID, Concat('DR ', a.DRNo) as DRNo,  concat('PO ',pono) as PONO,Concat('GR ', a.GRno) as GRNO,date_format(a.DRdate, '%m/%d/%Y') as DateDelivered, " & _
  " CustomerName,  a.isCancelled as Cancelled, A.isapproved as Approved," & _
 " a.DateCreated, A.LastUpdate, a.Creator as CreatedBy, a.remarks From DeliveryReceipt a JOIN Customers d On a.CustomerID=d.CustomerID where a.companyid='" & companyid & "'"
    Dim query2 As String = "  a.deliveryreceiptid desc"
    ...
    Private Sub readdetails(ByVal sql As String, ByVal b As Integer)
        ...
        Dim deliveryReceiptid As Integer = 0
        Dim ds As New DataSet
        connect()
        con.Open()
        ...
        End With
        con.Close()
        deliveryReceiptid = ds.Tables(0).Rows(0).Item("deliveryReceiptID")
        ''CANCEL COMMAND
        If b = 1 Then
            ...
            With command
                .CommandText = "update deliveryReceipt set isapproved=@isapproved, iscancelled=@iscancelled, cancelledby=@cancelledby, datecancelled=@datecancelled, " & _
               "lastupdate=@lastupdate where deliveryReceiptid=@deliveryReceiptid"
                With .Parameters
                    ...
                    .AddWithValue("@deliveryReceiptid", deliveryReceiptid)
                End With
                .Connection = con
                .ExecuteNonQuery()
        ...
        ElseIf b = 2 Then
            ...
            Dim command As New MySqlCommand
            With command
                .CommandText = "update deliveryReceipt set iscancelled=@iscancelled, isapproved=@isapproved, approvedby=@approvedby, dateapproved=@dateapproved, " & _
               "lastupdate=@lastupdate where deliveryReceiptid=@deliveryReceiptid"
                With .Parameters
                ...
                .Parameters.AddWithValue("@id", deliveryReceiptid.ToString("d6"))
            ...
            ''DELETE COMMAND
        ElseIf b = 4 Then
            Dim x As New deliveryreceiptentry(deliveryReceiptid)
            With x
                .toenable(False)
...
#Region "Controls and Commands"
    Private Sub Register_Click() Handles SOAddMenu.Click
        MainLauncher(0)
        Dim x As New deliveryreceiptentry(0)
        With x
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readdetails("SELECT * from deliveryReceipt WHERE deliveryReceiptid='" & Me.DataGridView1.Item(i, "deliveryReceiptid") & _
                           "'", 5)
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from deliveryReceipt WHERE deliveryReceiptid='" & Me.DataGridView1.Item(i, "deliveryReceiptid") & _
                         "'", "deleted.")
                saveEditDelete("DELETE from deliveryReceiptorders WHERE deliveryReceiptid='" & Me.DataGridView1.Item(i, "deliveryReceiptid") & _
                         "'", "wala")
    ...
    Private Sub CancelJobOrderButton_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles CancelJobOrderButton.Click
        Try
            Dim id As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("deliveryReceiptid")
            readdetails("select * from deliveryReceipt where deliveryReceiptid=" & id, 1)
        Catch ex As Exception
    ...
    Private Sub searchdetails()
        ...
        Try
            ...
            id = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("deliveryReceiptid")
            FunctionClass.loadtoGrid3("Grid_deliveryReceipt_Main_Details_01", id, Me.C1FlexGrid1)
...
#End Region
    ...
    Public Sub formatdatagrid()
        Try
            With Me.DataGridView1
                ...
                .Cols("deliveryReceiptid").Visible = False
                .Cols("DateDelivered").Visible = True
                .Cols("DRNo").Visible = True
    ...
    Private Function performSearchQuery(ByVal sortarrangement As String) As String
        ...
        Else
            If Me.DateFrom.Text <> Nothing Then
                If DateFrom.Text <> Nothing Then
                    RRDatequery = " and  date_format(a.DRDATE, '%Y/%m/%d')  between '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and  a.DRDATE >= '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If DateTo.Text <> Nothing Then
                    RRDatequery = " and a.DRDATE<= '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
        ...
        Return query1 & " and a.drno LIKE '%" & Val(Me.PONo.Text) & _
            "%' AND customername Like '%" & Me.CustomerName.Text &
             "%' AND a.pono like '%" & Me.PONoo.Text & _
              "%' AND a.grno Like '%" & Me.grno.Text & _
            "%' AND a.Creator Like '%" & Me.Creator.Text & "%' " & _
           isCancelledQuery & RRDatequery & "  ORDER BY a.drno" & sortarrangement
