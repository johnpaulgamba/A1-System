...
Public Class creditmemoEntry
    ...
    Public creditmemoid As Integer
    ...
    Public Sub New(ByVal id As Integer)
        ...
        ' loadingvaluestoComboBoxes("Select distinct creditterms from creditmemo", creditterms, "creditterms", "creditterms")
    ...
    Private Sub loadentries(ByVal id As Integer)
        Try
            ...
            Dim adapter As New MySqlDataAdapter("select * from creditmemo where creditmemoid=" & id, con)
            With adapter
                .Fill(ds)
                .Dispose()
            End With
            ...
            With ds.Tables(0).Rows(0)
                ...
                Me.SIDate.Value = .Item("cmdate")
                Me.creditmemoid = .Item("creditmemoid")
                Dim idx As Integer = .Item("cmno")
                Me.cmno.Text = idx.ToString("d6")
                Me.Remarks.Text = .Item("remarks")
                Me.CreditTerms.Text = .Item("terms")
                Me.Remarks.Text = .Item("remarks")
                Me.SIAmount.Text = .Item("grossamount")
                ...
                If tosave = False Then
                    loadtoGridNoFixed("Select a.itemid, a.salesinvoiceid, c.itemcode, upper(a.itemname),upper(a.uom), b.quantity,a.quantity,a.unitprice,a.discount,a.quantity*(a.unitprice-a.discount),a.remarks " & _
                          "from creditmemoorders a join salesinvoiceorders b on a.salesinvoiceid=b.salesinvoiceid join items c on a.itemid=c.itemid where " & _
    ...
    Private Sub loadcustomerdetails(ByVal id As Integer)
        ...
        End Try
    End Sub
    Private Sub InputButton3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Newcreditmemo.Click
        clear()
    End Sub
    Public Sub clear()
        'POReference.Text = ""
        ...
        toenable(True)
        tosave = True
        Dim drnox As Integer = searchRecord("select max(cmno) as Auto_Increment from creditmemo where companyid='" & companyid & "'", 1) + 1
        Me.cmno.Text = drnox.ToString("d6") 'POReference.Focus()
        ' Dim depositid As Integer = searchRecord("select max(creditmemoid) as id from creditmemo where branchid='" & companyid & "'", 1)
    ...
    Private Sub btnsave_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveJobOrder.Click
        ...
        'checkserialgrid()
        If tosave = True Then
            Dim depositid As Integer = searchRecord("select auto_increment from information_schema.tables where  table_schema='" & database & "' and table_name='creditmemo';", 1)
            savecommand("transaction_creditmemo_entry", depositid)
        Else
            savecommand("transaction_creditmemo_entry", creditmemoid)
        End If

    End Sub
    Private Function savecommand(ByVal a As String, ByVal depositid As Integer) As Boolean
        Dim creditmemoid As String = depositid
        ...
        Try
            ...
            With command
                ...
                .CommandType = CommandType.StoredProcedure
                With .Parameters
                    .AddWithValue("@creditmemoids", depositid)
                ...
                With mynewcommand
                    ...
                    If Me.tosave = False Then
                        .CommandText = "delete from creditmemoorders where creditmemoid='" & depositid & "'"
                        .Connection = con
                    ...
                    If Me.C1FlexGrid1.Rows.Count > 1 Then
                        .CommandText = "insert into `creditmemoorders` (`creditmemoid`,`salesinvoiceid`,`itemid`,`itemname`,`uom`,`quantity`,`unitprice`, `discount`, `remarks`)" & _
                    ...
                    .CommandText = "Update creditmemo  set isapproved=1,dateapproved=now(), iscancelled=0 where creditmemoid = '" & depositid & "'"
                    .Connection = con
                    .CommandType = CommandType.Text
                    .ExecuteNonQuery()
                ...
                Me.tosave = False
                Me.creditmemoid = depositid
                CreditMemoMainForm.LoadingForm()
                Return True
                Exit Function
            End With
    ...
    Private Sub InputButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintCreditMemo.Click
        Try
            ...
            With x
                .print("Reporting_creditmemo_Solo", "id", creditmemoid, 7, vattype, cur)
                .Text = "Print  preview of Credit Memo #" & creditmemoid
        ...
        Try
            ...
            If sagot = vbYes Then
                ...
                With command
                    ...
                    Else
                        .CommandText = "update creditmemo set isapproved=0,iscancelled=1, cancelledby='" & APCESMainForm.UserFullName.Text & "',lastupdate='" & "" & Now & "',datecancelled='" & Format(Now, "yyyy-MM-dd HH:mm:ss") & "' where creditmemoid='" & creditmemoid & "'"
                        .Connection = con
                        .ExecuteNonQuery()
                        ...
                        ' creditmemoMain.refreshgrid_click()
                        End
    ...
    Private Sub cmno_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmno.Validated
        If tosave = True Then
            connect()
            con.Open()
            command = New MySqlCommand("select * from creditmemo where cmno='" & cmno.Text & "' and companyid='" & companyid & "'", con)
    ...
    Private Sub InputButton2_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles InputButton2.Click
        Try
            loadtoGridNoFixed("Select a.itemid, a.salesinvoiceid, b.itemcode, upper(a.itemname),upper(a.uom), a.quantity,a.quantity-a.quantity,a.unitprice,a.discount,a.quantity*(a.unitprice-a.discount),a.remarks " & _
                                 "from salesinvoiceorders a join items b on a.itemid=b.itemid where a.salesinvoiceid='" & SIno.SelectedValue & "'", C1FlexGrid1)
            loadcustomerdetails(SIno.SelectedValue)
