...
Public Class SalesInvoiceMain
    ...
    Dim query1 As String = "Select a.SalesInvoiceID, Concat('SI ', a.SINO) as SINO,  Concat('DR ', a.DRNo) as DRNo,Concat('PO ', a.PONo) as PONO,date_format(a.SIDate, '%m/%d/%Y') as InvoiceDate,date_format(a.DueDate, '%m/%d/%Y') as DueDate, " & _
  " CustomerName, a.TaxCode, concat(currency,' ',format(VatableSales, 2)) as VatableSales,concat(currency,' ',format(VatAmount, 2)) as VAT,concat(currency,' ',format(GrossAmount, 2)) as Amount,  a.isCancelled as Cancelled, A.isapproved as Approved,a.Paid" & _
 " ,a.DateCreated, A.LastUpdate, a.Creator as CreatedBy, a.remarks From SalesInvoice a JOIN Customers d On a.CustomerID=d.CustomerID where a.companyid='" & companyid & "'"
    Dim query2 As String = "  a.SalesInvoiceid desc"
    ...
    Private Sub readdetails(ByVal sql As String, ByVal b As Integer)
        ...
        Dim SalesInvoiceid As Integer = 0
        Dim ds As New DataSet
        connect()
        con.Open()
        ...
        End With
        con.Close()
        SalesInvoiceid = ds.Tables(0).Rows(0).Item("SalesInvoiceID")
        ''CANCEL COMMAND
        If b = 1 Then
            ...
            With command
                .CommandText = "update SalesInvoice set isapproved=@isapproved, iscancelled=@iscancelled, cancelledby=@cancelledby, datecancelled=@datecancelled, " & _
               "lastupdate=@lastupdate where SalesInvoiceid=@SalesInvoiceid"
                With .Parameters
                    ...
                    .AddWithValue("@SalesInvoiceid", SalesInvoiceid)
                End With
                .Connection = con
                .ExecuteNonQuery()
        ...
        ElseIf b = 2 Then
            ...
            Dim command As New MySqlCommand
            With command
                .CommandText = "update SalesInvoice set iscancelled=@iscancelled, isapproved=@isapproved, approvedby=@approvedby, dateapproved=@dateapproved, " & _
               "lastupdate=@lastupdate where SalesInvoiceid=@SalesInvoiceid"
                With .Parameters
                ...
                .Parameters.AddWithValue("@id", SalesInvoiceid.ToString("d6"))
            ...
            ''DELETE COMMAND
        ElseIf b = 4 Then
            Dim x As New salesinvoiceEntry(SalesInvoiceid)
            With x
                .toenable(False)
...
#Region "Controls and Commands"
    Private Sub Register_Click() Handles SOAddMenu.Click
        MainLauncher(0)
        Dim x As New salesinvoiceEntry(0)
        With x
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readdetails("SELECT * from SalesInvoice WHERE SalesInvoiceid='" & Me.DataGridView1.Item(i, "SalesInvoiceid") & _
                           "'", 5)
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from SalesInvoice WHERE SalesInvoiceid='" & Me.DataGridView1.Item(i, "SalesInvoiceid") & _
                         "'", "deleted.")
                saveEditDelete("DELETE from SalesInvoiceorders WHERE SalesInvoiceid='" & Me.DataGridView1.Item(i, "SalesInvoiceid") & _
                         "'", "wala")
    ...
    Private Sub CancelJobOrderButton_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles CancelJobOrderButton.Click
        Try
            Dim id As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("SalesInvoiceid")
            readdetails("select * from SalesInvoice where SalesInvoiceid=" & id, 1)
        Catch ex As Exception
    ...
    Private Sub searchdetails()
        ...
        Try
            ...
            id = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("SalesInvoiceid")
            FunctionClass.loadtoGrid3("Grid_SalesInvoice_Main_Details_01", id, Me.C1FlexGrid1)
...
#End Region
    ...
    Public Sub formatdatagrid()
        Try
            With Me.DataGridView1
                ...
                .Cols("SalesInvoiceid").Visible = False
                .Cols("InvoiceDate").Visible = True
                .Cols("SINO").Visible = True
                .Cols("Amount").TextAlignFixed = TextAlignEnum.RightCenter
                .Cols("VAT").TextAlignFixed = TextAlignEnum.RightCenter
                .Cols("VatableSales").TextAlignFixed = TextAlignEnum.RightCenter
    ...
    Private Function performSearchQuery(ByVal sortarrangement As String) As String
        ...
        Else
            If Me.DateFrom.Text <> Nothing Then
                If DateFrom.Text <> Nothing Then
                    RRDatequery = " and  date_format(a.SIDate, '%Y/%m/%d')  between '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and  a.SIDate >= '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If DateTo.Text <> Nothing Then
                    RRDatequery = " and a.SIDate<= '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
        ...
        Return query1 & " and a.SINO LIKE '%" & Val(Me.SINo.Text) & _
            "%' AND customername Like '%" & Me.CustomerName.Text & _
            "%' AND a.SINo Like '%" & Me.SINo.Text & _
              "%' AND a.DRNo Like '%" & Me.DRno.Text & _
            "%' AND a.Creator Like '%" & Me.Creator.Text & "%' " & _
           isCancelledQuery & RRDatequery & "  ORDER BY a.SINO" & sortarrangement
