...
Public Class PaymentMainForm

    Dim query1 As String = "Select Paymentid, Concat('OR ', a.ORNo) as ORNo,date_format(ORDate, '%m/%d/%Y') as DatePaid, a.Tag as RefDocs, a.PayMethod as PaymentMethod,format(a.Amount,2) as AmountPaid," & _
    " A.Reference, CustomerName, a.isCancelled as Cancel,A.isapproved as Approved,a.Remarks, a.Creator as CreatedBy,a.DateCreated, A.LastUpdate From Payment a JOIN Customers d On a.CustomerID =d.CustomerID" & _
    " join company c on a.companyid=c.companyid WHERE a.CustomerID =d.CustomerID and c.companyid=a.companyid and a.Paymentid<> 0 and a.companyid='" & companyid & "'"
    Dim query2 As String = " a.Paymentid desc"
    ...
    Private Sub readdetails(ByVal sql As String, ByVal b As Integer)
        ...
        End With
        con.Close()
        Paymentid = ds.Tables(0).Rows(0).Item("PaymentID")
        ''CANCEL COMMAND
        If b = 1 Then
            ...
            With command
                .CommandText = "update Payment set isapproved=@isapproved, iscancelled=@iscancelled, cancelledby=@cancelledby,  " & _
               "lastupdate=@lastupdate where Paymentid=@Paymentid"
                With .Parameters
        ...
        ElseIf b = 2 Then
            ...
            Dim command As New MySqlCommand
            With command
                .CommandText = "update Payment set iscancelled=@iscancelled, isapproved=@isapproved, approvedby=@approvedby, dateapproved=@dateapproved, " & _
               "lastupdate=@lastupdate where Paymentid=@Paymentid"
                With .Parameters
                ...
                .Parameters.AddWithValue("@id", Paymentid.ToString("d6"))
            ...
            ''DELETE COMMAND
        ElseIf b = 4 Then
            Dim x As New PaymentEntry(Paymentid, False)
            With x
                .toenable(False)
        ...
        ElseIf b = 5 Then
            Dim x As New PaymentEntry(Paymentid, True)
            With x
                ' .toedit = True
                .Paymentid = Paymentid
                .tosave = False
...
#Region "Controls and Commands"
    Private Sub Register_Click() Handles SOAddMenu.Click
        MainLauncher(0)
        Dim x As New PaymentEntry(0, True)
        With x
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readdetails("SELECT * from Payment WHERE Paymentid='" & Me.DataGridView1.Item(i, "Paymentid") & _
                           "'", 5)
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from Payment WHERE Paymentid='" & Me.DataGridView1.Item(i, "Paymentid") & _
                         "'", "deleted.")
                saveEditDelete("DELETE from Paymentorders WHERE Paymentid='" & Me.DataGridView1.Item(i, "Paymentid") & _
                         "'", "wala")
    ...
    Private Sub CancelJobOrderButton_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles CancelJobOrderButton.Click
        Try
            Dim id As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("Paymentid")
            MessageBox.Show(Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("Paymentid"))
    ...
    Private Sub searchdetails()
        ...
        Try
            ...
            id = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("Paymentid")
            FunctionClass.loadtoGrid3("Grid_Payment_Main_Details_01", id, Me.C1FlexGrid1)
    ...
    Public Sub formatdatagrid()
        Try
            With Me.DataGridView1
                ...
                .Cols("Paymentid").Visible = False
                .Cols("ORDate").Visible = True
                .Cols("ORNo").Visible = True
                .Cols("AmountPaid").TextAlignFixed = TextAlignEnum.RightCenter
                ...
                .AutoSizeCol(.Cols("RefDocs").Index)
        ...
        Else
            If Me.DateFrom.Text <> Nothing Then
                If DateFrom.Text <> Nothing Then
                    RRDatequery = " and  date_format(a.ORDate, '%Y/%m/%d')  between '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and  a.ORDate >= '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If DateTo.Text <> Nothing Then
                    RRDatequery = " and a.ORDate<= '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and a.ORDate like '%%' "
                End If
            End If
        End If


        Return query1 & " and a.ORNo LIKE '%" & Val(Me.ORNO.Text) & _
            "%' AND customername Like '%" & Me.CustomerName.Text & _
            "%' AND a.tag Like '%" & Me.sino.Text & _
            "%' AND a.Creator Like '%" & Me.Creator.Text & "%' " & _
           isCancelledQuery & RRDatequery & "  ORDER BY a.ORNo" & sortarrangement
