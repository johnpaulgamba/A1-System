...
Public Class SalesOrderMain
    Dim query1 As String = "Select a.PurchaseorderID, Concat('', a.PONo) as PONo, date_format(a.POdate, '%m/%d/%Y') as DateOrdered, " & _
  " CustomerName,a.TaxCode, concat(a.currency,' ',format(GrossAmount, 2)) as Amount,  a.isCancelled as Cancelled, A.isapproved as Approved, a.Fulfilled," & _
 " a.DateCreated, A.LastUpdate, a.Creator as CreatedBy, a.remarks From Purchaseorder a JOIN Customers d On a.CustomerID=d.CustomerID where a.companyid='" & companyid & "'"
    Dim query2 As String = "  a.podate desc"
    ...
    Private Sub readdetails(ByVal sql As String, ByVal b As Integer)
        ...
        End With
        con.Close()
        PurchaseOrderid = ds.Tables(0).Rows(0).Item("PurchaseOrderID")
        ''CANCEL COMMAND
        If b = 1 Then
            ...
            With command
                .CommandText = "update PurchaseOrder set isapproved=@isapproved, iscancelled=@iscancelled, cancelledby=@cancelledby, datecancelled=@datecancelled, " & _
               "lastupdate=@lastupdate where PurchaseOrderid=@PurchaseOrderid"
                With .Parameters
                    ...
                    .AddWithValue("@PurchaseOrderid", PurchaseOrderid)
                End With
                .Connection = con
                .ExecuteNonQuery()
        ...
        ElseIf b = 2 Then
            ...
            Dim command As New MySqlCommand
            With command
                .CommandText = "update PurchaseOrder set iscancelled=@iscancelled, isapproved=@isapproved, approvedby=@approvedby, dateapproved=@dateapproved, " & _
               "lastupdate=@lastupdate where PurchaseOrderid=@PurchaseOrderid"
                With .Parameters
                ...
                '.Parameters.AddWithValue("@id", PurchaseOrderid.ToString("d6"))
            ...
            ''DELETE COMMAND
        ElseIf b = 4 Then
            Dim x As New SalesOrderEntry(PurchaseOrderid)
            With x
                .toenable(False)
...
#Region "Controls and Commands"
    Private Sub Register_Click() Handles SOAddMenu.Click
        MainLauncher(0)
        Dim x As New SalesOrderEntry(0)
        With x
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readdetails("SELECT * from purchaseorder WHERE purchaseorderid='" & Me.DataGridView1.Item(i, "purchaseorderid") & _
                           "'", 5)
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from purchaseorder WHERE purchaseorderid='" & Me.DataGridView1.Item(i, "purchaseorderid") & _
                         "'", "deleted.")
                saveEditDelete("DELETE from purchaseorderorders WHERE purchaseorderid='" & Me.DataGridView1.Item(i, "purchaseorderid") & _
                         "'", "wala")
    ...
    Private Sub CancelJobOrderButton_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles CancelJobOrderButton.Click
        Try
            Dim id As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("purchaseorderid")
            readdetails("select * from PurchaseOrder where PurchaseOrderid=" & id, 1)
        Catch ex As Exception
    ...
    Private Sub searchdetails()
        ...
        Try
            ...
            id = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("purchaseorderid")
            FunctionClass.loadtoGrid3("Grid_purchaseorder_Main_Details_01", id, Me.C1FlexGrid1)
    ...
    Public Sub formatdatagrid()
        Try
            With Me.DataGridView1
                ...
                .Cols("purchaseorderid").Visible = False
                .Cols("dateOrdered").Visible = True
        ...
        Else
            If Me.DateFrom.Text <> Nothing Then
                If DateFrom.Text <> Nothing Then
                    RRDatequery = " and  date_format(a.poDATE, '%Y/%m/%d')  between '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and  a.poDATE >= '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If DateTo.Text <> Nothing Then
                    RRDatequery = " and a.poDATE<= '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and a.poDATE like '%%' "
                End If
            End If
        End If


        Return query1 & " and a.pono LIKE '%" & Val(Me.PONo.Text) & _
            "%' AND customername Like '%" & Me.CustomerName.Text & _
              "%' AND a.taxcode Like '%" & Me.TaxCode.Text & _
              "%' AND a.remarks Like '%" & Me.Remarks.Text & _
            "%' AND a.Creator Like '%" & Me.Creator.Text & "%' " & _
           isCancelledQuery & RRDatequery & "  ORDER BY a.pono" & sortarrangement
