...
Public Class DebitMemoMainForm

    Dim query1 As String = "Select  DebitMemoid, Concat('DM ', a.DMNo) as DMNo, Concat('SI ', a.sino) as SIno, date_format(a.DMDate, '%m/%d/%Y') as DMDate, " & _
  "CustomerName,a.TaxCode,format(a.VatAmount,2) as Vat, format(GrossAmount, 2) as Amount,a.isCancelled as Cancelled,A.isapproved as Approved,a.Paid,a.AmountPaid,a.DateCreated," & _
  "A.LastUpdate,a.Creator as CreatedBy From DebitMemo a JOIN Customers d On a.CustomerID=d.CustomerID WHERE a.DebitMemoid<> 0 and a.companyid='" & companyid & "'"
    Dim query2 As String = "  a.DebitMemoid desc"
    ...
    Private Sub readdetails(ByVal sql As String, ByVal b As Integer)
        ...
        End With
        con.Close()
        DebitMemoid = ds.Tables(0).Rows(0).Item("DebitMemoID")
        ''CANCEL COMMAND
        If b = 1 Then
            ...
            With command
                .CommandText = "update DebitMemo set isapproved=@isapproved, iscancelled=@iscancelled, cancelledby=@cancelledby, datecancelled=@datecancelled, " & _
               "lastupdate=@lastupdate where DebitMemoid=@DebitMemoid"
                With .Parameters
                    ...
                    .AddWithValue("@DebitMemoid", DebitMemoid)
                End With
                .Connection = con
                .ExecuteNonQuery()
        ...
        ElseIf b = 2 Then
            ...
            Dim command As New MySqlCommand
            With command
                .CommandText = "update DebitMemo set iscancelled=@iscancelled, isapproved=@isapproved, approvedby=@approvedby, dateapproved=@dateapproved, " & _
               "lastupdate=@lastupdate where DebitMemoid=@DebitMemoid"
                With .Parameters
                    .AddWithValue("@approvedby", APCESMainForm.UserFullName.Text)
                ...
                .Parameters.AddWithValue("@id", DebitMemoid.ToString("d6"))
            ...
            ''DELETE COMMAND
        ElseIf b = 4 Then
            Dim x As New DebitMemoEntry(DebitMemoid)
            With x
                .toenable(False)
...
#Region "Controls and Commands"
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readdetails("SELECT * from DebitMemo WHERE DebitMemoid='" & Me.DataGridView1.Item(i, "DebitMemoid") & _
                           "'", 5)
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from DebitMemo WHERE DebitMemoid='" & Me.DataGridView1.Item(i, "DebitMemoid") & _
                         "'", "deleted.")
                saveEditDelete("DELETE from DebitMemoorders WHERE DebitMemoid='" & Me.DataGridView1.Item(i, "DebitMemoid") & _
                         "'", "wala")
    ...
    Private Sub CancelJobOrderButton_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles CancelJobOrderButton.Click
        Try
            Dim id As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("DebitMemoid")
            readdetails("select * from DebitMemo where DebitMemoid=" & id, 1)
        Catch ex As Exception
    ...
    Private Sub searchdetails()
        ...
        Try
            ...
            id = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("DebitMemoid")
            FunctionClass.loadtoGrid3("Grid_DebitMemo_Main_Details_01", id, Me.C1FlexGrid1)
...
#End Region
    ...
    Public Sub formatdatagrid()
        Try
            With Me.DataGridView1
                ...
                .Cols("DebitMemoid").Visible = False
                .Cols("DMDate").Visible = True
                .Cols("DMNo").Visible = True
                .Cols("Amount").TextAlignFixed = TextAlignEnum.RightCenter
                .Cols("VAT").TextAlignFixed = TextAlignEnum.RightCenter
                .Cols("CustomerName").Visible = True
                ...
                .Cols("Paid").TextAlignFixed = TextAlignEnum.CenterCenter
                '.Cols("Remarks").Width = 200
                .AutoSizeCol(.Cols("CustomerName").Index)
        ...
        Else
            If Me.DateFrom.Text <> Nothing Then
                If DateFrom.Text <> Nothing Then
                    RRDatequery = " and  date_format(a.DMDate, '%Y/%m/%d')  between '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and  a.DMDate >= '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If DateTo.Text <> Nothing Then
                    RRDatequery = " and a.DMDate<= '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and a.DMDate like '%%' "
                End If
            End If
        End If


        Return query1 & " and a.DMNo LIKE '%" & Val(Me.DMNo.Text) & _
            "%' AND customername Like '%" & Me.CustomerName.Text & _
            "%' AND a.sino Like '%" & Me.sino.Text & _
            "%' AND a.Creator Like '%" & Me.Creator.Text & "%' " & _
           isCancelledQuery & RRDatequery & "  ORDER BY a.DMNo" & sortarrangement
    ...
    Private Sub LoadingForm(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles SOShowAll.Click

    End Sub

    Private Sub Refresh_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles SoRefresh.Click, ReloadAllRecords.Click

    End Sub

    Private Sub ViewItemHistory_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles SOViewRecord.Click

    End Sub

    Private Sub DeleteRecord_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles SODeleteMenu.Click

    End Sub

    Private Sub EditRecord_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles SOEditMenu.Click

    End Sub

    Private Sub Register_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles SOAddMenu.Click

    End Sub

    Private Sub LastPageClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MoveToLast.Click

    End Sub

    Private Sub NextPageClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NextPage.Click

    End Sub

    Private Sub PreviousPageClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PreviousPage.Click

    End Sub

    Private Sub gotoFirst(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles First.Click
