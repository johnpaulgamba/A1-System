...
Public Class CreditMemoMainForm

    Dim query1 As String = "Select creditmemoid, Concat('CM ', a.CMNo) as CMNo, Concat('SI ', a.sino) as SIno, date_format(a.CMDate, '%m/%d/%Y') as CMDate, " & _
  ...
  "A.LastUpdate,a.Creator as CreatedBy From CreditMemo a JOIN Customers d On a.CustomerID=d.CustomerID WHERE a.CreditMemoid<> 0 and a.companyid='" & companyid & "'"
    Dim query2 As String = "  a.creditmemoid desc"
    ...
    Private Sub readdetails(ByVal sql As String, ByVal b As Integer)
        ...
        End With
        con.Close()
        creditmemoid = ds.Tables(0).Rows(0).Item("creditmemoID")
        ''CANCEL COMMAND
        If b = 1 Then
            ...
            With command
                .CommandText = "update creditmemo set isapproved=@isapproved, iscancelled=@iscancelled, cancelledby=@cancelledby, datecancelled=@datecancelled, " & _
               "lastupdate=@lastupdate where creditmemoid=@creditmemoid"
                With .Parameters
                    ...
                    .AddWithValue("@creditmemoid", creditmemoid)
                End With
                .Connection = con
                .ExecuteNonQuery()

                .Parameters.AddWithValue("@id", creditmemoid)
                .CommandText = "Inventory_creditmemo_recall"
        ...
        ElseIf b = 2 Then
            ...
            Dim command As New MySqlCommand
            With command
                .CommandText = "update creditmemo set iscancelled=@iscancelled, isapproved=@isapproved, approvedby=@approvedby, dateapproved=@dateapproved, " & _
               "lastupdate=@lastupdate where creditmemoid=@creditmemoid"
                With .Parameters
                ...
                .Parameters.AddWithValue("@id", creditmemoid.ToString("d6"))
                .CommandText = "Inventory_creditmemo_APPROVAL"
                .CommandType = CommandType.StoredProcedure
                .Connection = con
                .ExecuteNonQuery()
            ...
            ''DELETE COMMAND
        ElseIf b = 4 Then
            Dim x As New creditmemoEntry(creditmemoid)
            With x
                .toenable(False)
...
#Region "Controls and Commands"
    Private Sub Register_Click() Handles SOAddMenu.Click
        MainLauncher(0)
        Dim x As New creditmemoEntry(0)
        With x
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readdetails("SELECT * from creditmemo WHERE creditmemoid='" & Me.DataGridView1.Item(i, "creditmemoid") & _
                           "'", 5)
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from creditmemo WHERE creditmemoid='" & Me.DataGridView1.Item(i, "creditmemoid") & _
                         "'", "deleted.")
                saveEditDelete("DELETE from creditmemoorders WHERE creditmemoid='" & Me.DataGridView1.Item(i, "creditmemoid") & _
                         "'", "wala")
    ...
    Private Sub CancelJobOrderButton_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles CancelJobOrderButton.Click
        Try
            Dim id As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("creditmemoid")
            readdetails("select * from creditmemo where creditmemoid=" & id, 1)
        Catch ex As Exception
    ...
    Private Sub searchdetails()
        ...
        Try
            ...
            id = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("creditmemoid")
            FunctionClass.loadtoGrid3("Grid_creditmemo_Main_Details_01", id, Me.C1FlexGrid1)
    ...
    Public Sub formatdatagrid()
        Try
            With Me.DataGridView1
                ...
                .Cols("creditmemoid").Visible = False
                .Cols("CMDate").Visible = True
                .Cols("CMno").Visible = True
        ...
        Else
            If Me.DateFrom.Text <> Nothing Then
                If DateFrom.Text <> Nothing Then
                    RRDatequery = " and  date_format(a.cmdate, '%Y/%m/%d')  between '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' and '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and  a.cmdate >= '" & Format(DateFrom.Value, "yyyy/MM/dd") & "' "
                End If
            Else
                If DateTo.Text <> Nothing Then
                    RRDatequery = " and a.cmdate<= '" & Format(DateTo.Value, "yyyy/MM/dd") & "' "
                Else
                    RRDatequery = " and a.cmdate like '%%' "
                End If
            End If
        End If


        Return query1 & " and a.cmno LIKE '%" & Val(Me.CMNo.Text) & _
            "%' AND customername Like '%" & Me.CustomerName.Text & _
            "%' AND a.sino Like '%" & Me.sino.Text & _
            "%' AND a.Creator Like '%" & Me.Creator.Text & "%' " & _
           isCancelledQuery & RRDatequery & "  ORDER BY a.cmno" & sortarrangement
