...
Public Class HRIS_DepartmentMainForm
    Dim query1 As String = "SELECT DepartmentID, Department, Description, Active, Creator from Departments "
    Dim query2 As String = " Department"
        ...
        Try
            ...
            If b = 1 Then
                If reader.Read = True Then
                    Dim DepartmentID As Integer = reader.Item("DepartmentID")
                    Dim Department As String = reader.Item("Department")
                    Dim x As New HRIS_DepartmentRegistration
                    With x
                        .QuerytoString = overallQuery
                        .toSave = False
                        .EditLoad(DepartmentID)
                        .DepartmentID = DepartmentID
                        .Text = "Currently modifying : " & Department
...
#Region "Controls and Buttons"
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readingAllInfo("SELECT * from Departments WHERE DepartmentID='" & Me.DataGridView1.Item(i, "DepartmentID") & _
                           "'", 1)
        Catch ex As Exception
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            Dim i As Integer = Me.DataGridView1.RowSel
            Dim ID As Integer = Me.DataGridView1.Rows(i).Item("DepartmentID")
            Dim x As New HistoryUpdate
            With x
                .loaddata("Departments", "DepartmentID", ID)
                .Text = "Item history of " & Me.DataGridView1.Item(i, "Department")
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from Departments WHERE DepartmentID='" & Me.DataGridView1.Item(i, "DepartmentID") & _
                         "'", "deleted.")
                Refresh_Click()
            End If
        Catch ex As Exception
    ...
    Private Sub Refresh_Click() Handles SoRefresh.Click
        loadingDepartments(overallQuery)
    End Sub
...
#End Region
    ...
    Public Sub LoadingForm() Handles SOShowAll.Click
        ...
        loadingDepartments(overallQuery)
    End Sub
    Public Sub loadingDepartments(ByVal sql As String)
        Try
    ...
    Public Sub formatdatagrid()
        ...
        With Me.DataGridView1
            ...
            ' .Item(0, 1) = "ID"
            .Cols("Departmentid").Visible = False
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        overallQuery = performSearchQuery(Department.Text, Creator.Text, Description.Text,  OrderbyOf, sortarrangement, returnActiveInactive(RecordsActive.Checked, RecordsInactive.Checked, RecordsBoth.Checked))
        loadingDepartments(overallQuery)
    End Sub
    Private Function performSearchQuery(ByVal Classification As String, ByVal creator As String, ByVal description As String, _
        ...
        Dim query1 As String = "SELECT * from Departments WHERE"
        Classification = "  Department LIKE '%" & convertQuotes(Me.Department.Text) & "%'"
        ...
        Return query1 & Classification & creator & description & active & " Order by " & orderby & sortarrangement
    End Function
