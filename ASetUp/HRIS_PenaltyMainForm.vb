...
Public Class HRIS_PenaltySetUpMainForm
    Dim query1 As String = "SELECT * from HRIS_PenaltySetUp "
    Dim query2 As String = " PenaltyLevel"
...
#Region "Controls and Buttons"
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readingAllInfo("SELECT * from HRIS_PenaltySetUp WHERE PenaltyID='" & Me.DataGridView1.Item(i, "PenaltyID") & _
                           "'", 1)
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            With x
                ...
                .Text = "Item history of " & Me.DataGridView1.Item(i, "PenaltyID")
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from HRIS_PenaltySetUp WHERE PenaltyID='" & Me.DataGridView1.Item(i, "PenaltyID") & _
                         "'", "deleted.")
                Refresh_Click()
            End If
        Catch ex As Exception
    ...
    Private Sub Refresh_Click() Handles SoRefresh.Click, ReloadAllRecords.Click
        loadingHRIS_Penalty(overallQuery)
    End Sub
...
#End Region
        ...
        Try
            ...
            If b = 1 Then
                If reader.Read = True Then
                    Dim PenaltyID As Integer = reader.Item("PenaltyID")
                    Dim Penalty As String = reader.Item("PenaltyLevel")
                    Dim x As New HRIS_PenaltySetUpRegistration
                    With x
                        .QuerytoString = overallQuery
                        .toSave = False
                        .EditLoad(PenaltyID)
                        .PenaltyID = PenaltyID
                        .Text = "Currently modifying : " & Penalty
                        .StartPosition = FormStartPosition.CenterParent
                        .ShowDialog()
                    End With
                    con.Close()
    ...
    Public Sub LoadingForm() Handles SOShowAll.Click
        ...
        loadingHRIS_Penalty(overallQuery)
    End Sub
    Public Sub loadingHRIS_Penalty(ByVal sql As String)
        Try
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        If Me.OrderBY.Text.ToLower = "classification" Then Orderbyof = "Penalty" Else Orderbyof = Trim(Me.OrderBY.Text.Replace(" ", ""))
        overallQuery = performSearchQuery(Penalty.Text, Creator.Text, Description.Text, "", DateCreated.Text, LastUpdate.Text, Orderbyof, sortarrangement, returnActiveInactive(RecordsActive.Checked, RecordsInactive.Checked, RecordsBoth.Checked))
        loadingHRIS_Penalty(overallQuery)
    End Sub
    ...
    Private Function performSearchQuery(ByVal Classification As String, ByVal creator As String, ByVal description As String, _
        ...
        Dim query1 As String = "SELECT * from HRIS_PenaltySetUp WHERE"
        Classification = "  PenaltyLevel LIKE '%" & convertQuotes(Me.Penalty.Text) & "%'"
        creator = " AND Creator LIKE '%" & convertQuotes(Me.Creator.Text) & "%' "
        Return query1 & Classification & creator & changesmade & active & " Order by " & orderby & sortarrangement
    End Function
