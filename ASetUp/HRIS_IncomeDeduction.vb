...
Public Class HRIS_IncomeDeduction
...
#End Region
    ...
    Private Sub SaveJobOrder_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveJobOrder.Click
        ...
        '  Try
        If RecordDoExists("Select * from companypayrollsetup where companyid='" & CompanyName.SelectedValue & "'") = True Then
            Dim vbresponse = MessageBox.Show("Payroll Setup for this company has already been set-up." & vbNewLine & "Are you sure you want to continue with the update?", "Confirm Update", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
        ...
        Try
            ...
            For x = 1 To y
                With C1flexgrid1.Rows(x)
                    ...
                    If .Item("paytype") = Nothing Then Continue For
                    If .Item("Payid") = Nothing Then

                        saveEditDelete("Insert into companypayrollsetup (`companyid`,`paytype`,`payrate`,`compute`,`government`,`income`,`deduction`,`SemiMonthly`,`Weekly`,`13th`,`Order`,`Active`,`remarks`,`creator`,`changesmade`,`Amount`) values " & _
                                       " ('" & CompanyName.SelectedValue & "','" & .Item("paytype").ToString.ToUpper & "','" & Val(.Item("payrate")) & "','" & toint(.Item("Compute")) & _
                                       "','" & toint(.Item("government")) & "','" & toint(.Item("income")) & "','" & toint(.Item("Deduction")) & "','" & toint(.Item("SemiMonthly")) & "','" & toint(.Item("Weekly")) & "','" & toint(.Item("13th")) & "','" & Val(.Item("Order")) & _
                                       "','1','" & .Item("Remarks") & "','" & APCESMainForm.UserFullName.Text & "','Originally created: " & convertQuotes(.Item("PayType")) & vbCrLf & _
                                  "by " & convertQuotes(APCESMainForm.UserFullName.Text) & " on " & Now & "@ " & _
                                 convertQuotes(APCESMainForm.ComputerName.Text) & "','" & Val(.Item("Amount")) & "')", "wala")

                    ElseIf .Item("Payid") <> Nothing Then
                        saveEditDelete("update companypayrollsetup SET paytype= '" & _
                                   convertQuotes(.Item("Paytype").ToString) & _
                                "', PayRate='" & Val(.Item("payrate")) & _
                                "', Active='" & toint(.Item("Active")) & _
                                "', compute='" & toint(.Item("compute")) & _
                                "', government='" & toint(.Item("government")) & _
                                "', income='" & toint(.Item("income")) & _
                                "', deduction='" & toint(.Item("deduction")) & _
                                "', semimonthly='" & toint(.Item("SemiMonthly")) & _
                                "', Weekly='" & toint(.Item("weekly")) & _
                                 "', `13th`='" & toint(.Item("13th")) & _
                                "', `Order`='" & Val(.Item("Order")) & _
                                "', `Amount`='" & Val(.Item("Amount")) & _
                                "', ChangesMade=Concat('Updated to: " & MySqlHelper.EscapeString(.Item("PayType")) & vbCrLf & _
                                 "by " & _
                                 APCESMainForm.UserFullName.Text & " at " & Now & _
                                 "@ " & APCESMainForm.ComputerName.Text & _
                                 vbCrLf & vbCrLf & "-->'" & _
                                 ", ChangesMade) WHERE payid='" & .Item("PayiD") & "'", "wala")
                    End If

                End With
            Next
            FunctionClass.loadtoGrid("Select Payid, PayType, PayRate,Compute,Government,Income,Deduction,Active,`SemiMonthly`,`Weekly`,`13th`,`Order`,Remarks from companypayrollsetup where companyid='" & CompanyName.SelectedValue & "'", C1flexgrid1)
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Sub HRIS_EarningSetup_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        FunctionClass.loadtoGrid("Select Payid, PayType, PayRate,Compute,Government,Income,Deduction,Active,`SemiMonthly`,`Weekly`,`13th`,`Order`,Remarks from companypayrollsetup where companyid='" & companyid & "'", C1flexgrid1)
    End Sub
    ...
    Private Sub C1flexgrid1_AfterEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.RowColEventArgs) Handles C1flexgrid1.AfterEdit
        ...
        For x = 1 To y
            With C1flexgrid1.Rows(x)
                If .Item("payrate") = Nothing Then .Item("payrate") = 0
    ...
    Private Sub Load_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Load.Click
        Try
            'FunctionClass.loadtoGrid("Select Payid, PayType, PayRate,Compute,Government,Income,Deduction,Active,`Order`,Remarks from companypayrollsetup where companyid='1'", C1flexgrid1)
            FunctionClass.loadtoGrid("Select Payid, PayType, PayRate,Compute,Government,Income,Deduction,Active,`SemiMonthly`,`Weekly`,`13th`,`Order`,Remarks from companypayrollsetup where companyid='" & CompanyName.SelectedValue & "'", C1flexgrid1)
        Catch ex As Exception
            '  MessageBox.Show(ex.Message)
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            Dim ID As Integer = Me.C1flexgrid1.Rows(i).Item("payid")
            Dim x As New HistoryUpdate
            With x
                .loaddata("companypayrollsetup", "payID", ID)
