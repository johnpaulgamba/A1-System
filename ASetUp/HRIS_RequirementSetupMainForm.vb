...
Imports System.IO
Public Class HRIS_RequirementSetupMainForm

    Dim query1 As String = " Select a.RequirementID as RequirementID, " & _
        " Concat('REQ ', a.RequirementID) as Reference, " & _
        " a.Requirement as Requirements, " & _
        " a.Optional, " & _
        " a.Creator " & _
        " From HRIS_Requirement_Setup a where a.requirementid <> 0 "
    Dim query2 As String = " a.Requirement  "
    Dim QUERY3 As String = " GROUP BY a.RequirementID "
...
#Region "Controls and Commands"
    Private Sub Register_Click() Handles SOAddMenu.Click
        Try
            Dim x As New HRIS_EmployeeRegistration_Requirements_SetUp(0)
            With x
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            Dim ID As Integer = Me.DataGridView1.Rows(i).Item("RequirementID")
            Dim x As New HistoryUpdate
            With x
                .loaddata("HRIS_Requirement_Setup", "RequirementID", ID)
...
#End Region
    ...
    Public Sub formatdatagrid()
        With Me.DataGridView1
            ...
            .Cols("RequirementID").Visible = False
            ColumnVisible()
        End With
    End Sub
    ...
    Private Function performSearchQuery(ByVal sortarrangement As String) As String
        ...
        If Me.ActiveBoth.Checked = True Then isPosted = "" Else isPosted = " AND b.Active=" & Convert.ToSingle(Me.ActiveYes.Checked)
        Dim requirements As String = " AND requirement LIKE '%" & convertQuotes(Me.requirement.Text) & "%'"
        Dim descriptions As String = " AND description LIKE '%" & convertQuotes(Me.description.Text) & "%'"
        Dim Order As String = " ASC"
        '  Dim OrderByOf As String = Me.OrderBY.Text.Replace(" ", "")
        Return query1 & requirements & descriptions & isPosted & " ORDER BY a.requirementid"
    End Function
    ...
    Private Sub EditCommand_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles EditCommand.Click
        Try
            ...
            Dim i As Integer = Me.DataGridView1.Rows(Me.DataGridView1.RowSel).Item("RequirementID")
            Dim x As New HRIS_EmployeeRegistration_Requirements_SetUp(i)
            With x
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
