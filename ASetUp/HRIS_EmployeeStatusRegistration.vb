...
Public Class hris_EmploymentStatusRegistration
    ...
    Public EmploymentStatusID As Integer
    ...
    Private Sub tostartNewRecord()
        toSave = True
        With Me
            .Text = "EmploymentStatus Entry"
            .Status.Checked = True
            .EmploymentStatus.Text = Nothing
    ...
    Public Sub EditLoad(ByVal a As Integer)
        loadDS("SELECT * from hris_EmploymentStatus where EmploymentStatusID='" & a & "'")
    End Sub
    ...
    Private Sub SaveRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        Try
            If Trim(Me.EmploymentStatus.Text) = "" Then MessageBox.Show("EmploymentStatus is required!", "Required field", MessageBoxButtons.OK, MessageBoxIcon.Stop) : Me.EmploymentStatus.Focus() : Exit Sub
            ...
            If toSave = True Then
                If RecordDoExists("SELECT * from hris_EmploymentStatus where EmploymentStatus='" & Me.EmploymentStatus.Text & "'") = True Then
                    MessageBox.Show("The EmploymentStatus exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            Else
                If RecordDoExists("SELECT * from hris_EmploymentStatus where EmploymentStatus='" & Me.EmploymentStatus.Text & "' AND EmploymentStatusID<> '" & Me.EmploymentStatusID & "'") = True Then
                    MessageBox.Show("The EmploymentStatus exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            End If
            If toSave = True Then
                saveEditDelete("INSERT into hris_EmploymentStatus (EmploymentStatus, Description,  Active, Creator, ChangesMade) VALUES ( '" & _
                                    convertQuotes(Me.EmploymentStatus.Text) & _
                                 "', '" & convertQuotes(Me.Description.Text) & _
                                  "', '" & Convert.ToSingle(Me.Status.Checked) & _
                                   "', '" & APCESMainForm.UserFullName.Text & _
                                  "', 'Originally created: " & convertQuotes(EmploymentStatus.Text) & vbCrLf & _
                                 ...
                                 convertQuotes(APCESMainForm.ComputerName.Text) & _
                                  "')", "saved")
            Else
                saveEditDelete("update hris_EmploymentStatus SET EmploymentStatus= '" & _
                                    convertQuotes(Me.EmploymentStatus.Text) & _
                                 "', Description='" & convertQuotes(Me.Description.Text) & _
                                 "', Active='" & Convert.ToSingle(Me.Status.Checked) & _
                                 "', ChangesMade=Concat('Updated to: " & MySqlHelper.EscapeString(Me.EmploymentStatus.Text.ToUpper) & vbCrLf & _
                                  "Active:" & Me.Status.Checked & vbCrLf & _
                                  "by " & _
                                  APCESMainForm.UserFullName.Text & " at " & Now & _
                                  "@ " & APCESMainForm.ComputerName.Text & _
                                  vbCrLf & vbCrLf & "-->'" & _
                                  ", ChangesMade) WHERE EmploymentStatusID='" & Me.EmploymentStatusID & "'", "updated..")
            End If
            ...
            If IsNotError = False Then
                ...
                hris_EmploymentStatusMainForm.loadingEmploymentStatuss(QuerytoString)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Function RecordDoExists(ByVal a As String) As Boolean
        ...
        End Try
    End Function
    Private Sub EmploymentStatus_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles EmploymentStatus.Validating
        If toSave = True Then
            If RecordDoExists("SELECT * from hris_EmploymentStatus where EmploymentStatus='" & Me.EmploymentStatus.Text & "'") = True Then
                MessageBox.Show("The EmploymentStatus exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                e.Cancel = True
