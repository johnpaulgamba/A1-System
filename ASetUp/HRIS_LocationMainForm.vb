...
Public Class HRIS_LocationMainForm
    Dim query1 As String = "SELECT LocationID,Location, Description,Active,Creator,ChangesMade from hris_location "
    Dim query2 As String = " Location"
        ...
        Try
            ...
            If b = 1 Then
                If reader.Read = True Then
                    Dim LocationID As Integer = reader.Item("LocationID")
                    Dim Location As String = reader.Item("Location")
                    Dim x As New HRIS_LocationRegistration
                    With x
                        .QuerytoString = overallQuery
                        .toSave = False
                        .EditLoad(LocationID)
                        .LocationID = LocationID
                        .Text = "Currently modifying : " & Location
...
#Region "Controls and Buttons"
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readingAllInfo("SELECT * from hris_location WHERE LocationID='" & Me.DataGridView1.Item(i, "LocationID") & _
                           "'", 1)
        Catch ex As Exception
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            With x
                ...
                .Text = "Item history of " & Me.DataGridView1.Item(i, "LocationID")
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from hris_location WHERE LocationID='" & Me.DataGridView1.Item(i, "LocationID") & _
                         "'", "deleted.")
                Refresh_Click()
            End If
        Catch ex As Exception
...
#End Region
    ...
    Public Sub LoadingForm() Handles SOShowAll.Click
        ...
        loadingLocations(overallQuery)
    End Sub
    Public Sub loadingLocations(ByVal sql As String)
        Try
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        overallQuery = performSearchQuery(Location.Text, Creator.Text, Description.Text, "", "", "", OrderbyOf, sortarrangement, returnActiveInactive(RecordsActive.Checked, RecordsInactive.Checked, RecordsBoth.Checked))
        loadingLocations(overallQuery)
    End Sub
    Private Function performSearchQuery(ByVal Classification As String, ByVal creator As String, ByVal description As String, _
        ...
        Dim query1 As String = "SELECT * from hris_location WHERE"
        Classification = "  Location LIKE '%" & convertQuotes(Me.Location.Text) & "%'"
