...
Public Class HRIS_GroupMainForm
    Dim query1 As String = "SELECT * from hris_Group "
    Dim query2 As String = " GroupName"
        ...
        Try
            ...
            If b = 1 Then
                If reader.Read = True Then
                    Dim GroupID As Integer = reader.Item("GroupID")
                    Dim Group As String = reader.Item("Group")
                    Dim x As New HRIS_GroupRegistration
                    With x
                        .QuerytoString = overallQuery
                        .toSave = False
                        .EditLoad(GroupID)
                        .GroupID = GroupID
                        .Text = "Currently modifying : " & Group
...
#Region "Controls and Buttons"
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readingAllInfo("SELECT * from hris_Group WHERE GroupID='" & Me.DataGridView1.Item(i, "GroupID") & _
                           "'", 1)
        Catch ex As Exception
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            With x
                ...
                .Text = "Item history of " & Me.DataGridView1.Item(i, "GroupID")
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from hris_Group WHERE GroupID='" & Me.DataGridView1.Item(i, "GroupID") & _
                         "'", "deleted.")
                Refresh_Click()
            End If
        Catch ex As Exception
...
#End Region
    ...
    Public Sub LoadingForm() Handles SOShowAll.Click
        ...
        loadingGroups(overallQuery)
    End Sub
    Public Sub loadingGroups(ByVal sql As String)
        Try
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        overallQuery = performSearchQuery(Group.Text, Creator.Text, Description.Text, "", "", "", OrderbyOf, sortarrangement, returnActiveInactive(RecordsActive.Checked, RecordsInactive.Checked, RecordsBoth.Checked))
        loadingGroups(overallQuery)
    End Sub
    Private Function performSearchQuery(ByVal Classification As String, ByVal creator As String, ByVal description As String, _
        ...
        Dim query1 As String = "SELECT * from hris_Group WHERE"
        Classification = "  GroupName LIKE '%" & convertQuotes(Me.Group.Text) & "%'"
