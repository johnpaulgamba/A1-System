...
Public Class HRIS_ReligionRegistration
    ...
    Public Sub EditLoad(ByVal a As Integer)
        loadDS("SELECT * from hris_Religion where ReligionID='" & a & "'")
    End Sub
    ...
    Private Sub SaveRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        Try
            If Trim(Me.Religion.Text) = "" Then MessageBox.Show("Religion is required!", "Required field", MessageBoxButtons.OK, MessageBoxIcon.Stop) : Me.Religion.Focus() : Exit Sub
            ...
            If toSave = True Then
                If RecordDoExists("SELECT * from hris_Religion where Religion='" & Me.Religion.Text & "'") = True Then
                    MessageBox.Show("The Religion exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            Else
                If RecordDoExists("SELECT * from hris_Religion where Religion='" & Me.Religion.Text & "' AND ReligionID<> '" & Me.ReligionID & "'") = True Then
                    MessageBox.Show("The Religion exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Exit Sub
                End If
            End If
            If toSave = True Then
                saveEditDelete("INSERT into hris_Religion (Religion, Description,  Active, Creator, ChangesMade) VALUES ( '" & _
                                    convertQuotes(Me.Religion.Text) & _
                                 "', '" & convertQuotes(Me.Description.Text) & _
                                  "', '" & Convert.ToSingle(Me.Status.Checked) & _
                                   "', '" & APCESMainForm.UserFullName.Text & _
                                  "', 'Originally created: " & convertQuotes(Religion.Text) & vbCrLf & _
                                 ...
                                 convertQuotes(APCESMainForm.ComputerName.Text) & _
                                  "')", "saved")
            Else
                saveEditDelete("update hris_Religion SET Religion= '" & _
                                    convertQuotes(Me.Religion.Text) & _
                                 "', Description='" & convertQuotes(Me.Description.Text) & _
                                 "', Active='" & Convert.ToSingle(Me.Status.Checked) & _
                                 "', ChangesMade=Concat('Updated to: " & MySqlHelper.EscapeString(Me.Religion.Text.ToUpper) & vbCrLf & _
                                  "Active:" & Me.Status.Checked & vbCrLf & _
                                  "by " & _
                                  APCESMainForm.UserFullName.Text & " at " & Now & _
                                  "@ " & APCESMainForm.ComputerName.Text & _
                                  vbCrLf & vbCrLf & "-->'" & _
                                  ", ChangesMade) WHERE ReligionID='" & Me.ReligionID & "'", "updated..")
            End If
            ...
            If IsNotError = False Then
                ...
                HRIS_ReligionMainForm.loadingReligions(QuerytoString)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Function RecordDoExists(ByVal a As String) As Boolean
        ...
        End Try
    End Function
    Private Sub Religion_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Religion.Validating
        If toSave = True Then
            If RecordDoExists("SELECT * from hris_Religion where Religion='" & Me.Religion.Text & "'") = True Then
                MessageBox.Show("The Religion exist already.", "Exist already.", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                e.Cancel = True
