...
Public Class SSSTableMainForm
    Dim query1 As String = "SELECT Bracketid, format(RangeFrom,3)as Range_From, format(RangeTo,3) as Range_To, format(SalaryCredit,3) as Salary_Credit, format(EE,3) as EE_Share,format(ER,3) as ER_Share, format(EC,3) as EC_Share,format(EE,3) as EE_Total, format(ERTotal,3) as ER_Total,format(Total,3) as SSS_Total, Creator  from SSSTable "
    Dim query2 As String = " bracketid"
        ...
        Try
            ...
            If b = 1 Then
                If reader.Read = True Then
                    Dim SSSTableID As Integer = reader.Item("SSSTableID")
                    Dim SSSTable As String = reader.Item("SSSTable")
                    'Dim x As New SSSTableRegistration
                    ' With x
                    '.QuerytoString = overallQuery
                    ' .toSave = False
                    '.EditLoad(SSSTableID)
                    ' .SSSTableID = SSSTableID
                    ' .Text = "Currently modifying : " & SSSTable
...
#Region "Controls and Buttons"
    ...
    Private Sub EditRecord_Click() Handles SOEditMenu.Click
        Try
            ...
            readingAllInfo("SELECT * from hris_SSSTable WHERE SSSTableID='" & Me.DataGridView1.Item(i, "SSSTableID") & _
                           "'", 1)
        Catch ex As Exception
    ...
    Private Sub ViewItemHistory_Click() Handles SOViewRecord.Click
        Try
            ...
            With x
                ...
                .Text = "Item history of " & Me.DataGridView1.Item(i, "SSSTableID")
                .StartPosition = FormStartPosition.CenterParent
                .ShowDialog()
            End With
        Catch ex As Exception
    ...
    Private Sub DeleteRecord_Click() Handles SODeleteMenu.Click
        Try
            ...
            If answer = MsgBoxResult.Yes Then
                Dim i As Integer = Me.DataGridView1.RowSel
                saveEditDelete("DELETE from hris_SSSTable WHERE SSSTableID='" & Me.DataGridView1.Item(i, "SSSTableID") & _
                         "'", "deleted.")
                Refresh_Click()
            End If
        Catch ex As Exception
...
#End Region
    ...
    Public Sub LoadingForm() Handles SOShowAll.Click
        ...
        loadingSSSTables(overallQuery)
    End Sub
    Public Sub loadingSSSTables(ByVal sql As String)
       Try
    ...
    Public Sub formatdatagrid()
        ...
        With Me.DataGridView1
            ...
            .Item(0, 1) = "ID"
            .Cols("BracketID").TextAlign = TextAlignEnum.RightCenter
            .Cols("Range_From").TextAlign = TextAlignEnum.RightCenter
            .Cols("Range_To").TextAlign = TextAlignEnum.RightCenter
            .Cols("Salary_Credit").TextAlign = TextAlignEnum.RightCenter
            .Cols("ER_Share").TextAlign = TextAlignEnum.RightCenter
            .Cols("EE_Share").TextAlign = TextAlignEnum.RightCenter
            .Cols("EC_Share").TextAlign = TextAlignEnum.RightCenter
            .Cols("ER_Total").TextAlign = TextAlignEnum.RightCenter
            .Cols("EE_Total").TextAlign = TextAlignEnum.RightCenter
            .Cols("SSS_Total").TextAlign = TextAlignEnum.RightCenter
    ...
    Private Sub SearchRecords_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchRecords.Click
        ...
        overallQuery = performSearchQuery(SSSTable.Text, Creator.Text, Description.Text, ChangesMade.Text, DateCreated.Text, LastUpdate.Text, OrderbyOf, sortarrangement, returnActiveInactive(RecordsActive.Checked, RecordsInactive.Checked, RecordsBoth.Checked))
        loadingSSSTables(overallQuery)
    End Sub
    Private Function performSearchQuery(ByVal Classification As String, ByVal creator As String, ByVal description As String, _
        ...
        Dim query1 As String = "SELECT * from SSSTable WHERE"
        Classification = "  "
        creator = "  "
        description = " "
        changesmade = " "
