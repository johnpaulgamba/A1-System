...
Public Class joborderticket
    Sub New(ByVal id As Integer)
        ...
        If id = 0 Then Me.jodate.value = now
    ...
    Private Sub loadentries(ByVal id As Integer)
        Try
            ...
            Dim adapter As New mysqldataadapter("select * from joborderticket where joborderticketid=" & id, con)
            With adapter
                .fill(ds)
                .dispose()
            End With
            ...
            With ds.tables(0).rows(0)
                ...
                Me.joborderticketid = .item("joborderticketid")
                Me.technician.text = .item("technician")
                Me.footer.text = .item("footer")
                Me.landmark.text = .item("landmark")
                Me.tosave = False
                Me.sereference.text = "jo  " & Me.joborderticketid.tostring("d10")
                loadtogridnofixed("select a.itemid, itemname, b.uom,  quantity,   " & _
                           "  remarks from items a " & _
                           " join joborderticketorders b on a.itemid=b.itemid and joborderticketid=" & id, Me.c1flexgrid1)
    ...
    Private Sub saverecord_click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveRecord.Click
        ...
        If tosave = True Then
            Dim depositid As Integer = searchRecord("select auto_increment from information_schema.tables where  table_schema='" & database & "' and table_name='joborderticket';", 1)
            savecommand("transaction_joborderticket_entry", depositid)
        Else
            savecommand("transaction_joborderticket_entry", Me.joborderticketid)
        End If
    ...
    Private Function savecommand(ByVal a As String, ByVal depositid As Integer) As Boolean
        ...
        Try
            ...
            With command
                ...
                With .parameters
                    .addwithvalue("@jodates", Me.jodate.value)
                    .addwithvalue("@deliverydates", Me.deliverydate.value)
                    .addwithvalue("@joborderticketids", depositid)
                ...
                With mynewcommand
                    If Me.tosave = False Then
                        .commandtext = "delete from joborderticketorders where joborderticketid=" & depositid
                        .connection = con
                        .commandtype = commandtype.text
                        .executenonquery()
                    End If
                    .commandtext = "insert into `joborderticketorders` (`joborderticketid`,`itemid`,`uom`,`quantity`, `remarks`)" & _
                ...
                Me.joborderticketid = depositid
                Me.sereference.text = "jo " & depositid.tostring("d10")
                Me.tosave = False
                joborderticketmainform.refreshgrid_click()
                Return True
                Exit Function
            End With
        Catch ex As exception
    ...
    Private Sub c1flexgrid1_beforeedit(ByVal sender As System.Object, ByVal e As C1.Win.C1FlexGrid.RowColEventArgs) Handles C1FlexGrid1.BeforeEdit
        ...
        End If
    End Sub
    Private joborderticketid As Integer = 0
    ...
    Private Sub inputbutton2_click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles InputButton2.Click
        Try
            ...
            With x
                .print("reporting_joborderticket_solo", "id", Me.joborderticketid, 7)
                .Text = "print preview of se " & Me.joborderticketid.ToString("d10")
                .StartPosition = FormStartPosition.CenterParent
