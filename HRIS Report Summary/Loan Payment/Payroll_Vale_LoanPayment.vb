...
Public Class Payroll_Vale_LoanPayment
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click
        Try
            ...
            If Me.monthly.Checked = True Then
                filtersearchquery = s1 & " and date_format(d.payrolldate,'%Y-%m-%d') = '" & Format(Me.PayrollDate.Value, "yyyy-MM-dd") & "'"
                printReports("Reporting_Vale_Loan", filtersearchquery, 1)
            ElseIf Me.individual.Checked = True Then
                ...
                filtersearchquery = payrolldatequery & s4
                printReports("Reporting_Vale_Loan", filtersearchquery, 2)
            End If
            MainLauncher(0)
    ...
    Public Sub printReports(ByVal a As String, ByVal b As String, ByVal c As Integer)
        ...
        Try
            ...
            Else
                If c = 1 Then
                    Dim crpt As New Reporting_ValeLoan_Monthly
                    Dim inclusivedate, registerdate As CrystalDecisions.CrystalReports.Engine.TextObject
                    With crpt
                        registerdate = .ReportDefinition.Sections(0).ReportObjects("text7") : registerdate.Text = "Payroll Date " & Format(PayrollDate.Value, "MM/dd/yyyy")
                        inclusivedate = .ReportDefinition.Sections(4).ReportObjects("text6") : inclusivedate.Text = "TOTAL " & Format(PayrollDate.Value, "MM/dd/yyyy")
                        .SetDataSource(ds.Tables(0))
                        Me.CrystalReportViewer2.ReportSource = crpt
                        Me.CrystalReportViewer2.Refresh()
                    End With
                ElseIf c = 2 Then
                    Dim crpt As New Reporting_ValeLoan_Individual
                    Dim inclusivedate, registerdate As CrystalDecisions.CrystalReports.Engine.TextObject
                    With crpt
                        registerdate = .ReportDefinition.Sections(0).ReportObjects("text5") : registerdate.Text = "Date From " & Format(payrolldatefrom.Value, "MMM dd yyyy") & " to " & Format(payrolldateto.Value, "MMM dd yyyy")
                        inclusivedate = .ReportDefinition.Sections(4).ReportObjects("text7") : inclusivedate.Text = "Total " & Format(payrolldatefrom.Value, "MM/dd/yyyy") & "-" & Format(payrolldateto.Value, "MM/dd/yyyy")
                        .SetDataSource(ds.Tables(0))
                        Me.CrystalReportViewer2.ReportSource = crpt
                        Me.CrystalReportViewer2.Refresh()
                    End With
